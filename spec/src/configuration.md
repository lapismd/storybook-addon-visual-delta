...
| VD-CONF-008 | Project browser configuration MUST be a non-empty unique subset of `chromium`, `firefox`, and `webkit`, defaulting to only `chromium`. Visual test failure mode MUST be `warn` or `strict`, defaulting to `warn`; an explicit CLI value wins over `VISUAL_DELTA_FAILURE_MODE`, project workflow, and the built-in default. The built-in changed-pixel allowance MUST be `0.063%`; the independent built-in per-pixel `diffThreshold` MUST also be `0.063`. |
