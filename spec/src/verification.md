...
| `VD-API-001`, `VD-API-002`, `VD-API-003`, `VD-API-004`, `VD-API-005`, `VD-API-006`, `VD-API-007`, `VD-API-008`, `VD-API-009`, `VD-API-010` | `package.json`, `scripts/check-npm-release.mjs`, `src/node/cli.ts`, `src/node/init-scaffold.ts`, `src/node/middleware.ts`, `src/node/capture-runner.ts`, `src/node/baseline-migration.ts`, `src/node/story-facts.ts`, `src/shared/story-facts.ts`, `src/manager/visual-filters.ts`, `src/node/run-hub.ts` | CLI, capture-runner, migration, story-facts, filter, run-hub, package dry-run, and host middleware tests | Conforming |
