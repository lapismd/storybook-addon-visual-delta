...
## Downstream create-only mutation repair

Mira's 2026-08-03 create-missing workflow reproduced two host mutation
failures. First, middleware omitted the host's resolved snapshot directory and
`nested-import` path mode, so the worker interpreted 122 existing nested
baselines as missing flat targets and broadened the create-only capture to all
132 selected stories. The run was stopped before completion; staged output was
discarded and no host baseline changed. Forwarding the resolved baseline
identity reduced browser capture to the ten genuinely missing Indentation
stories.

That exact run then exposed Playwright's missing-snapshot exit after it had
written all ten requested PNGs. The transaction correctly refused to apply a
non-zero worker result, but the UI could only report `Baselines: create failed`.
The primary writer now uses the same fail-closed write-then-verify contract as
interaction capture: every requested missing target must exist before a second
updates-disabled replay can pass. Focused mutation tests cover host option
forwarding, explicit writer overrides, complete primary-set recovery, partial
write rejection, and interaction behavior. The repaired downstream UI action
captured and verified exactly ten stories, applied ten PNGs and their story
...
## Live status-batch and run-presentation repair

Mira's 2026-08-04 nine-story run completed with middleware reporting `done`,
`9/9`, and `childActive: false`, but the existing manager tab retained a
running footer and the selected story failed with
`importers[path] is not a function`. The live Storybook index named
`./stories/demo/Comprehensive.stories.ts`, while Vite's generated importer map
had dropped that recently rewritten CSF path together with two other files from
the same status batch. A fresh Storybook process contained all four paths and
rendered the story, proving an HMR/index synchronization defect rather than a
component import error or live capture process.

Conformance requires multi-story review requests to write each CSF file once,
the manager to reconcile retained running presentation against middleware
status until terminal, and the panel footer to expose Stop even when preview
rendering failed. Focused source, reconciliation, and footer component tests
plus live Mira status-batch importer inspection and browser acceptance record
the repair; no visual baseline is created or refreshed.

## Non-modal running-log cancellation repair

Release run
[`31048269389`](https://github.com/lapismd/storybook-addon-visual-delta/actions/runs/31048269389)
reproduced a Storybook `PopoverProvider` interaction regression in both wide
and narrow ARM64 panel acceptance. Opening the streamed progress log made the
surrounding document inert, so the footer's separately rendered **Stop** action
remained visible but every pointer attempt was intercepted until timeout.

The progress log now uses a labelled non-modal popover that retains Escape and
outside-pointer dismissal without installing an underlay or focus trap.
Component coverage opens the log before invoking Stop, and compare-only panel
acceptance proves the same sequence at wide and narrow viewport sizes. This
repair does not change panel baseline pixels or authorize snapshot updates.

## ARM64 release failure ledger

Release run
[`31051801634`](https://github.com/lapismd/storybook-addon-visual-delta/actions/runs/31051801634)
confirmed that the preceding ARM64 panel repair passed in both layouts, while
ARM64 manager acceptance exposed two further contract regressions. The custom
non-modal log surface no longer inherited the trigger's normalized log-line
name, so exact-story comparison acceptance could not identify the expanded
log. Separately, status reconciliation treated the synchronously published
baseline-create progress as orphaned middleware state and cleared it while the
originating HTTP request was still pending.

The repair keeps an internal count of locally active baseline-write requests so
HMR reconciliation clears only orphaned presentation, not live work. The log
surface and trigger share one normalized accessible label. Focused unit and
manager acceptance must prove the active-request lifetime, the pre-dispatch
progress bar, story-switch isolation, terminal hydration boundary, and dynamic
log discovery before the release failure is closed. No baseline mutation or
snapshot update is authorized by this ledger.
...
### Pull request and default-branch CI

Standalone package workflow `.github/workflows/visual-delta-ci.yml` runs on
pull requests, pushes to the default `main` branch, and pushes to the retained
compatibility branch `master`. Every trigger runs the same jobs (compare-only;
no snapshot updates):
