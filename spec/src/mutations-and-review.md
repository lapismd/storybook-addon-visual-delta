...
| VD-MUT-003 | A successful baseline write MUST invalidate prior comparison evidence for written stories. Storybook UI baseline writes and direct CLI writes with explicit `--update-status` MUST set only those stories to pending review; a direct CLI baseline write without that flag MUST preserve existing review metadata. The coordinated filename migration MUST remove legacy and canonical derived sidecars and invalidate the selected affected-test cache so browser-only paths cannot reuse platform-qualified passing evidence. It MUST NOT change unrelated stories. |
...
| VD-MUT-008 | A host MAY configure a story-source formatter command. Visual Delta MUST pass each staged changed source to that command on standard input, substitute the exact absolute source path into its declared arguments, and use standard output as the final source. Formatting MUST complete for every touched source before any source or static-index write begins. A formatter launch failure, non-zero exit, or empty output for non-empty input MUST fail the whole source mutation without partial writes. Without this option, source mutations MUST retain their existing dependency-free behavior. |
...
| --------------------- | -------------------------------------------- | ----------------------------------------------------- | ----------------------------------------- | ------------------------------- |
| Create baseline       | Frozen story IDs, create-only mode, approval | New requested PNGs and story wiring                   | Invalidate affected sidecars              | UI or `--update-status`: pending; otherwise unchanged |
| Update baseline       | Frozen story IDs, overwrite mode, approval   | Requested PNGs and wiring                             | Invalidate affected sidecars              | UI or `--update-status`: pending; otherwise unchanged |
...
When the host configures a story-source formatter, formatting is part of this
in-memory staging phase rather than a post-write hook. The formatter receives
the candidate source on standard input and returns the complete formatted
source on standard output. Visual Delta does not invoke a shell and requires a
`{filePath}` argument placeholder so formatters such as Prettier can resolve the
correct parser and repository configuration. All formatted candidates must
succeed before Visual Delta writes any touched source or synchronizes static
index metadata.
