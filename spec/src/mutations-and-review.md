...
Auto-accept MAY set approved only after a fresh authoritative Chromium outcome of `passed` or `changed-within-tolerance` from Diff Chromium / Story, or from Testing Module **Run Diff** last-run passes. It requires project opt-in (`workflow.autoAcceptLiveStoryComparisons`) and remains separate from baseline writes and Update status.
