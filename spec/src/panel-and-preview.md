...
Opening split zoom MUST follow project `previewSplitZoomDefault` (`fit` or `100%`), including when a follow-up INIT replaces built-in defaults after config load. A user-edited zoom MUST survive later INIT for the same story. Placement pad activation MUST reopen with the project default rather than always forcing Fit. The live comparison frame projects the measured Storybook root to the baseline capture width by adding the difference between capture and measured iframe widths. Only the root width is constrained: fixed and max-width subjects retain their natural width, while percentage, flex, grid, and container-width-responsive subjects reflow before the same Fit or `100%` scale is applied to both sides. The current manager iframe still defines the visible split slot.
