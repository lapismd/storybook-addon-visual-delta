...
Soft hide removes overlay or split DOM but retains the selected target and placement. Selecting the same placement again restores it. Hard clear resets comparison state when the story changes, the selected target disappears, or Storybook leaves Canvas mode.
...
Selecting an interaction remounts and replays the story to the exact capture point. The preview parks play until selection changes or clears. The panel MUST clear stale park state on story change and MUST recover it across an expected manager remount.
