...
| VD-UI-004 | Named modes MUST apply their exact Storybook globals. The mode dropdown MUST be the sole selector for Default and named-mode baselines, MUST omit a separate `Mode` label, and MUST show the resolved baseline thumbnail beside every name that has one. A selected mode MUST survive the same-story INIT and remount caused by applying its globals, keep its matching baseline selected, and fall back to Default only when that mode or baseline is no longer available. Interaction rows MUST represent `{ id, label, src }`, support deterministic replay, and distinguish discovered points from wired baselines. Replay MUST publish readiness for the exact parked generation before geometry or comparison work begins. An unwired discovered point MUST expose creation for that exact point when the backend permits writes. The bounded interaction list MUST own vertical overflow so every row and action remains reachable. |
...
- A primary baseline thumbnail and image-backed mode selector
