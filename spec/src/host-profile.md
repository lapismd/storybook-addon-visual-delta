...
| VD-HOST-006 | Portable behavior belongs in the package, including the package-owned self-test Storybook (`.storybook` in the Visual Delta sibling repo) and Visual Delta panel/manager acceptance fixtures. The panel screenshot harness MUST resolve its committed `-chromium-darwin.png` references independently of the operating system running the test; a CI runner MUST NOT derive a new expected baseline filename from its platform. Repository layout, Svelte source writers, generator approval gates, **product** catalog stories, and UI catalog port supervision belong to the host. Duplicate contract logic MUST converge on shared package helpers. |
