...
| VD-HOST-006 | Portable behavior belongs in the package, including the package-owned self-test Storybook (`.storybook` in the Visual Delta sibling repo) and Visual Delta panel/manager acceptance fixtures. The panel screenshot harness MUST use Playwright's platform-qualified reference identity. Each operating system that runs the panel screenshot suite MUST have its own reviewed, committed references; a runner MUST NOT compare against a reference created on a different operating system. Repository layout, Svelte source writers, generator approval gates, **product** catalog stories, and UI catalog port supervision belong to the host. Duplicate contract logic MUST converge on shared package helpers. |
