...
| VD-CAP-005 | Baseline images MUST display at CSS size using their device scale factor. Overlay and split alignment MUST reconstruct measured body, root, and subject insets. A component geometry mismatch MUST be reported only when the settled live iframe viewport matches the baseline capture viewport; a manager-preview measurement from another viewport MUST clear or suppress that warning. Only painted borders contribute to reconstructed geometry: a side whose computed border style is `none` or `hidden` MUST contribute zero width regardless of the width serialized by the CSSOM. |
| VD-CAP-006 | Live HTML comparison MUST remain diagnostic. Exact-story Diff Browser, Testing Module, and command-line comparisons are authoritative and MUST execute the same packaged Playwright suite through the resolved canonical capture runner. An authoritative middleware path MUST NOT launch a host-local Playwright browser or label a host-local capture with canonical provenance. Equivalent exact-story and scoped requests MUST share static inputs, configured-browser selection, thresholds, interaction/mode behavior, outcome classification, and canonical Linux/ARM64 capture profile. Baselines MUST be compared only with captures for the same browser target and matching profile; platform and architecture are provenance rather than baseline dimensions. |
...
A mismatch between baseline CSS dimensions and the same settled live capture target at the same viewport produces a geometry warning. The live side MUST use the subject-and-overlay union whenever the authoritative capture did; comparing an overlay baseline only with the subject box is invalid. A manager preview whose iframe dimensions differ from the baseline capture viewport does not provide comparable component geometry and MUST clear or suppress that warning. An inferred difference between dimensions and configured alignment produces an alignment warning. Warnings do not change the pixel outcome.
...
Diff HTML MUST label itself as diagnostic and MUST NOT approve, fail, create, update, or delete a baseline. Diff Browser MUST submit an exact `visual-delta test --story-id` job to the resolved capture runner and consume the sidecar and diagnostic PNGs produced by the same suite used for command-line runs. Diff Browser and scoped or command-line Playwright MUST therefore use the same thresholds, browser configuration, static Storybook, interaction or named-mode capture behavior, and outcome classifier for equivalent inputs.
