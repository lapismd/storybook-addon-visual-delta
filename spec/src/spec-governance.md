...
| ID         | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| VD-GOV-001 | Markdown under `spec/src/` MUST be the canonical Visual Delta contract for humans and agents. Repository-owned configuration MUST map Visual Delta policy and requirement identifiers onto reusable shared validators while excluding auxiliary `VD-GAP-*` labels from unknown-requirement checks; generated output, implementation, tests, and non-canonical documentation MUST NOT override it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| VD-GOV-004 | The configured spec-first gate MUST cover portable production code and repository-owned Visual Delta host integration. Tests, fixture stories, ordinary catalog stories, visual baselines, derived artifacts, and generated output MUST NOT trigger or satisfy the gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| VD-GOV-007 | The configured repository-layout validator MUST require exactly `AGENTS.md`, `DEVELOPMENT.md`, `README.md`, and `CHANGELOG.md` as package-root Markdown. The obsolete `specs/` tree and package-root historical contract or plan files MUST NOT exist. Links to normative content MUST target `spec/src/`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| VD-GOV-016 | Public package release intent MUST be recorded with Changesets. A pull request that changes portable public runtime source or the package manifest MUST include a Changeset file, or an explicit empty Changeset when no version bump is required. A merge to `main` with pending Changesets MUST open or update a Version Packages pull request that applies version and changelog updates. After that Version Packages pull request merges with no pending Changesets, continuous integration MUST create the exact `vX.Y.Z` tag matching the package version when the tag is absent, MUST leave an existing tag for the same commit unchanged, MUST leave an existing tag for a different commit unchanged when the package version did not change, MUST fail closed when a bumped version's tag already points at a different commit, and MUST NOT publish to npm from the versioning workflow. Ordinary non-versioning merges to `main` MUST NOT create release tags. The tag push MUST use a credential that can trigger the npm publish workflow; the default Actions `GITHUB_TOKEN` is insufficient because it cannot start other workflows.                                                                                                                                                                                                                       |
| VD-GOV-008 | A dependency-audit remediation that changes the Visual Delta package manifest or resolved dependency graph MUST upgrade each reported vulnerable package to a patched release and verify clean `deno task audit:dependencies` and `deno task audit:runtime` results. It MUST NOT create, replace, or delete visual baselines. |
| VD-GOV-009 | A public npm release MUST originate only from an exact stable `vX.Y.Z` tag matching the package version, pass the release validation gates, verify that the packed artifact retains its executable CLI bin, and wait for protected GitHub Environment approval. Normal publication MUST use npm trusted publishing with GitHub OIDC and provenance, never a registry token. The release MUST install the published package on a clean runner and verify its Sigstore provenance with `npm audit signatures`; the verifier MUST bind the tarball SHA-512 through npm's canonical scoped-package PURL subject. Only `v0.0.1` MAY use a one-time bootstrap token, isolated to a separate protected Environment; that job MUST configure the public registry through an ephemeral user config containing only an environment-variable token reference, MUST still publish provenance, and the token MUST be revoked before the next release.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| VD-GOV-011 | Repository-owned package tooling MUST use Deno `2.9.5` as its canonical runtime. Node.js `24.15.0` MAY remain only for explicit Node, npm, Playwright, Storybook, and GitHub Action compatibility boundaries. Every JavaScript action referenced by repository workflows MUST use a Node.js 24 runtime, and workflow validation MUST reject an action ref outside the audited Node.js 24 allowlist. |
| VD-GOV-012 | The repository CI image MUST be published to `ghcr.io/lapismd/storybook-addon-visual-delta-ci` only by a manually dispatched default-branch workflow and MUST remain publicly pullable without registry credentials so the built-in consumer runner needs no GHCR setup. A publication MUST build Linux AMD64 and ARM64 from the reviewed Dockerfile, pin Node.js `24.15.0`, npm `12.0.2`, Deno `2.9.5`, mdBook `0.5.4`, and Playwright Chromium, Firefox, and WebKit `1.61.1`, reject `latest` or an existing image tag as its audit tag, verify anonymous audit-tag inspection, and verify both native architectures. Only after both native smoke jobs pass, it MUST emit a versioned capture-profile manifest containing the multi-platform and ARM64 child digests, the actual browser and tool versions reported by the native ARM64 container, a content-derived manifest hash of that container's fonts, and the canonical locale, time-zone, viewport, scale, and rendering settings. Publication MAY update `latest` for generic tooling, but canonical visual consumers MUST use the immutable ARM64 digest. The workflow MAY grant only `contents: read` and `packages: write` and MUST NOT create or update a baseline.                                                                                                                                     |
| VD-GOV-013 | Repository-owned package-tooling jobs MUST use authenticated CI-image job containers, minimal permissions, Bash, and the root-owned `HOME=/root` required by Firefox, without reinstalling image-supplied toolchains or browsers. The container declaration MUST set `HOME=/root`, and each run step that launches Firefox MUST restore `HOME=/root` at step scope because GitHub Actions can replace the container value with its uid-1001-owned runner home. Root HOME MUST NOT be set at workflow or job scope so the host Docker client retains its runner-owned configuration path. A container job that reads checkout VCS history MUST register `$GITHUB_WORKSPACE` as safe under that root-owned HOME after checkout; this includes the npm release package gate because `spec:first` reads Git. Specification, package, npm publication, provenance, and image-build jobs MUST remain on stable x64 runners. Panel, manager, browser-matrix, release visual acceptance, and canonical baseline capture MUST run on `ubuntu-24.04-arm` with the immutable ARM64 capture-profile image. The npm release workflow MUST split x64 package gates from ARM64 visual gates and require both before publishing. A checked-out dependency graph still requires `deno task ci:install`. ARM64 visual jobs MUST be canaried successfully before becoming required. |
| VD-GOV-014 | The package Storybook MUST publish as a static, read-only GitHub Pages site at `https://lapismd.github.io/storybook-addon-visual-delta/` after a push to the default branch and on explicit manual dispatch. Its build job MUST use the authenticated x64 Node 24 CI image, Bash, root-owned container HOME, a frozen dependency install, and `deno task build-storybook`; it MAY read Pages metadata but MUST NOT receive Pages write or OIDC permission, MUST force the package-owned baseline fixtures so an optional sibling host checkout cannot enter the public artifact, and MUST verify `storybook-static/index.html`, `iframe.html`, and `index.json` before uploading only `storybook-static` with the official Node 24 Pages artifact action. A separate `github-pages` deployment job MUST depend on that artifact, use a stable GitHub-hosted x64 runner, grant only `pages: write` and `id-token: write`, and deploy with the official Node 24 Pages action. The deployment MUST NOT execute visual comparisons or create, replace, delete, or approve a baseline. The package README MUST link to the stable Pages URL.                                                                                                                                                                                                                                         |
| VD-GOV-015 | The compare-only Visual Delta CI workflow MUST run the same package, panel, browser-matrix, and manager jobs for pull requests, pushes to the default `main` branch, and pushes to the retained compatibility branch `master`. None of those triggers MAY enable snapshot updates or baseline mutations.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| VD-GOV-017 | Repository agent guidance MUST resolve colocated LapisMD siblings through explicit `lapismd-workspace.json` declarations, keep sibling repositories outside this Deno workspace, preserve portable published manifests, and require built-export providers to be rebuilt before consumer validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- `deno task spec:lint`
- `deno task spec:validate`
- `deno task spec:build`
- `deno task spec:serve`
- `deno task spec:first`

The complete check also runs configured repository-specific script and CI-image lanes. Shared checker regression tests remain owned by `@lapismd/spec-validator`. It runs near the start of `deno task check`, before implementation typechecking and browser validation.

On a pull request, the required **Visual Delta Spec First / Validate Visual Delta specification** check compares the exact base and head revisions. Its checkout fetches complete history and writes the workspace safe-directory entry after checkout, under the same root-owned HOME used when the checker invokes Git. Repository settings MUST keep that check required before merge. The workflow obtains Node 24.15.0 and Deno 2.9.5, and the prebuilt mdBook 0.5.4 binary from the repository CI image; it does not compile mdBook during a workflow run.

**Visual Delta CI** (`.github/workflows/visual-delta-ci.yml`) runs package typecheck and Vitest, panel, browser-matrix, and manager acceptance on pull requests and pushes to the default `main` branch or retained compatibility branch `master`. Spec First remains the contract merge gate; repository settings SHOULD enable the Visual Delta CI checks as required once they are stable on the default branch.
...
out of ordinary package jobs. Its Deno dependency cache is warmed from the reviewed
`package.json`, `deno.json`, and `deno.lock` with lifecycle scripts disabled, and mdBook
...
job-container `HOME` override cannot change the resolved Deno version. Image
...
dependency, Dockerfile, Node.js, npm, Deno, mdBook, Playwright, browser, or font
...
The portable package is publicly released to `https://registry.npmjs.org`.
Authors record release intent with Changesets on pull requests that change
public package surface. Merges to `main` with pending Changesets open or update
a Version Packages pull request. After that Version Packages pull request merges
and no Changesets remain, the release versioning workflow creates the exact
stable `vX.Y.Z` tag that matches `package.json` when appropriate; it does not
publish. Ordinary merges without a Version Packages commit must not create
tags.

The npm publish workflow accepts only pushed `v*` tags; its release guard
rejects a tag that does not exactly equal the stable package version, a private
package, an unexpected package identifier, or incorrect public-registry or
repository metadata. It runs the specification, build, typecheck, unit,
dependency-audit, package dry-run, and compare-only browser gates before
publication.

The package gate verifies that the packed artifact still exposes an executable
`visual-delta` command. The `npm` GitHub Environment MUST require reviewer
...
`NPM_BOOTSTRAP_TOKEN`. Its publish step writes an ephemeral npm user config
containing the literal `${NODE_AUTH_TOKEN}` reference and scopes that config to
the publish command; it never writes the secret value. After that release,
