...
| VD-GOV-010 | The Linux panel-baseline capture workflow MUST run only through manual dispatch, use read-only repository permission, and capture only missing platform-qualified Linux panel references. It MUST neither commit nor push baselines. After a compare-only verification pass, it MUST upload the exact Linux PNGs and their checksums as a review artifact. |
