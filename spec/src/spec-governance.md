...
On a pull request, the required **Visual Delta Spec First / Validate Visual Delta specification** check compares the exact base and head revisions. Repository settings MUST keep that check required before merge. The workflow obtains Node 24.15.0, pnpm 10.32.1, and the prebuilt mdBook 0.5.4 binary from the repository CI image; it does not compile mdBook during a workflow run.
